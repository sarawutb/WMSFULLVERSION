...
import '../tran_out_product_screen/create_barcode_screen/select_printer.dart';

class SelectPrinterMoniterPrinter extends StatefulWidget {
  const SelectPrinterMoniterPrinter({Key? key}) : super(key: key);

  @override
  _SelectPrinterMoniterPrinterState createState() =>
      _SelectPrinterMoniterPrinterState();
}

class _SelectPrinterMoniterPrinterState
    extends State<SelectPrinterMoniterPrinter> {
  // List<dynamic> _list = [];
      ...
      await RequestAssistant.getRequestHttpResponse(
        ...
        if (response.statusCode == 200) {
          ...
          List<CancelListPrinterModel> list = (responseJson as List)
              ...
              .toList();
          cPrinter.updatelistPrinterMonitorPrinter(list: list);
        }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () {
        _controller.clearlistPrinterMonitorPrinter();
        ...
        body: Consumer<ControllerCancelListPrinter>(
          builder: (context, value, child) {
            List<CancelListPrinterModel> _list =
                value.listPrinterMonitorPrinter;
                        ...
                        child: ListTile(
                          ...
                          title: Column(
                            ...
                            children: [
                              ...
                              // if ((_list[index].allProblem!).isNotEmpty)
                              //   if ((_list[index].allProblem!).length >= 2)
                              //     Text(
                              //       "${(_list[index].allProblem![0])}${(_list[index].allProblem![1])}",
                              //       style: _styledefult.copyWith(
                              //           fontWeight: FontWeight.bold),
                              //     ),
                              // if ((_list[index].allProblem!).length == 1)
                              //   Text(
                              //     "${(_list[index].allProblem![0])}",
                              //     style: _styledefult.copyWith(
                              //         fontWeight: FontWeight.bold),
                              //   ),
                            ],
                                  ...
                                  child: GestureDetector(
                                    onTap: () async {
                                      var res = await Navigator.push(
                                          ...
                                          MaterialPageRoute(
                                            builder: (context) =>
                                                CancelListPrinterMoniterPrinter(
                          ...
                          //         : Icons.print,
                          //     color: !_list[index]["offline"] ? Colors.green : gray,
                          //   ),
                          // ),
                        ),
                      )
...
class CancelListPrinterMoniterPrinter extends StatefulWidget {
  const CancelListPrinterMoniterPrinter(
      {Key? key,
      // required this.list,
  ...
  final int index;
  @override
  State<CancelListPrinterMoniterPrinter> createState() =>
      _CancelListPrinterMoniterPrinterState();
}

class _CancelListPrinterMoniterPrinterState
    extends State<CancelListPrinterMoniterPrinter> {
  // bool statuspage = true;
  ...
  Future removeprinter(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          // value.listPrinterMonitorPrinter[widget.index].jobs!.removeAt(index);
        });
      } else {
        showAlertDialogText(
      ...
      child: Consumer<ControllerCancelListPrinter>(
        builder: (context, value, child) => Scaffold(
          appBar: AppBar(
            ...
            elevation: 0,
            title: Text(
              "ค้างพิมพ์ (${value.listPrinterMonitorPrinter[widget.index].jobs!.length}) ${widget.status ? "ออฟไลน์" : "ออนไลน์"}",
              style: _styledefult,
            ),
          ),
          body: Container(
            child: value
                    .listPrinterMonitorPrinter[widget.index].jobs!.isNotEmpty
                ? ListView.builder(
                    itemCount: value
                        .listPrinterMonitorPrinter[widget.index].jobs!.length,
                    itemBuilder: (context, index) => Container(
                      ...
                      decoration: BoxDecoration(
                        color:
                            ...
                            //             value
                            //                 .listPrinterMonitorPrinter[widget.index]
                            //                 .jobs![index]
                            //                 .userPrint) ||
                      ...
                      child: Padding(
                          ...
                          child: ListTile(
                            title: Text(
                              'ชื่อไพล์ : ' +
                                  value.listPrinterMonitorPrinter[widget.index]
                                      .jobs![index].name!,
